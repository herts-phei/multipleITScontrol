...
tibble_data <- tibble_data %>%
  ...
  mutate(
    score = case_when(
      Period == "Pre-intervention period" & group_var == "treatment" ~ rnorm(n(), mean = 20, sd = 2),
      Period == "Intervention 1) Smart-Lock Upgrade" & group_var == "treatment" ~ rnorm(n(), mean = 30, sd = 2),
      Period == "Intervention 2) Predictive Maintenance Algorithm" & group_var == "treatment" ~ rnorm(n(), mean = 35, sd = 2),
      group_var == "control" ~ rnorm(n(), mean = 20, sd = 2))
    )
